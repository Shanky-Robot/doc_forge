...
class WebSearchMCPServer {
    ...
    if (req.method === 'POST' && req.url === '/tools/call') {
      ...
      req.on('end', async () => {
        try {
          ...
          let result;
          if (payload.name === 'web-search' || payload.name === 'full-web-search') {
            result = await this.handleWebSearch(this.validateAndConvertArgs(args));
            ...
            result.results.forEach((searchResult, idx) => {
              ...
              if (searchResult.fullContent) {
                ...
                responseText += `\n**Full Content:**\n${content}\n`;
              } else if (searchResult.contentPreview) {
  ...
  async run(): Promise<void> {
    ...
    httpServer.listen(3000, () => {
      console.log('HTTP wrapper listening on port 3000 for /tools/call endpoints');
    });
